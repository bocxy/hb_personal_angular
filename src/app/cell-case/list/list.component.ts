...
export class ListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.getAllEmpDCellCase();

   
    this.activeRoute.paramMap.subscribe(params => {
      this.id = params.get('id');
      this.deleteDCellCase;
  ...
  getAllEmpDCellCase(): void {
    const id = 123; // Replace with the actual employee ID
    this.employeeService.getAllEmpDCellCase(id).subscribe(
      (response) => {
  ...
  deleteDCellCase(id: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        ...
        this.employeeService.deleteDCellCase(id).subscribe(
          () => {
            // Data successfully deleted, no specific 'success' property
            this.openDialog(true, 'Exam data deleted successfully!');
            this.getAllEmpDCellCase(); // Refresh the data after successful deletion
