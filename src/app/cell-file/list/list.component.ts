...
export class ListComponent implements OnInit {
  ...
  columnsToDisplay = ['sno', 'fileNo', 'natureOfCase', 'modeOfCharge', 'noOfPersons', 'currentStatus', 'action'];
  ...
  constructor(public dialog: MatDialog, private snackbar: MatSnackBar, private router: Router, private employeeService:EmployeeService, private activeRoute: ActivatedRoute, ) { }
  ...
  ngOnInit(): void {
   ...
   this.getAllDCellCaseFile();

   this.activeRoute.paramMap.subscribe(params => {
    this.id = params.get('id');
    this.deleteDCellFile;
  ...
  getAllDCellCaseFile(): void {
    const id = 123; // Replace with the actual employee ID
    this.employeeService.getAllDCellCaseFile(id).subscribe(
      (response) => {
  ...
  deleteDCellFile(id: number) {
    // Open the confirmation dialog
    const dialogRef = this.dialog.open(DialogComponent, {
      width: '500px',
      data: {
        message: 'Are you sure to delete this DCell data?',
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        ...
        this.employeeService.deleteDCellFile(id).subscribe(
          () => {
            // Data successfully deleted, no specific 'success' property
            this.openDialog(true, 'Exam data deleted successfully!');
            this.getAllDCellCaseFile(); // Refresh the data after successful deletion
