...
export class ListComponent implements OnInit {
  ...
  columnsToDisplay = ['sno', 'cadreCode', 'cadreName', 'cadreCategory', 'bdResNumber', 'bdResDate', 'action'];
  ...
  ngOnInit(): void {
    ...
    this.getAllCgaDetails();

    this.activeRoute.paramMap.subscribe(params => {
      this.id = params.get('id');
      this.deleteCgaDetails;
 ...
 applyTypeFilter() {
  ...
  getAllCgaDetails(): void {
    const id = 123; // Replace with the actual employee ID
    this.employeeService.getAllCgaDetails(id).subscribe(
      (response) => {
      ...
      if (result === true) {
        ...
        this.employeeService.deleteCgaDetails(id).subscribe(
          () => {
            ...
            this.getAllCgaDetails(); // Refresh the data after successful deletion
