...
export interface AdditionalRows {
  sno: number;
  regSerialNo: string;
  applCertificateFilePath: string;
...
export class ListComponent implements OnInit {
  ...
  columnsToDisplay = ['sno', 'regSerialNo', 'applCertificateFilePath', 'dateOfApplication', 'eligibleStatus', 'appointmentStatus', 'action'];
  ...
  ngOnInit(): void {
    ...
    this.getAllCgaRegister();

    this.activeRoute.paramMap.subscribe(params => {
      this.id = params.get('id');
      this.deleteCgaRegister;
  ...
  getAllCgaRegister(): void {
    const id = 123; // Replace with the actual employee ID
    this.employeeService.getAllCgaRegister(id).subscribe(
      (response) => {
  ...
  deleteCgaRegister(id: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        ...
        this.employeeService.deleteCgaDetails(id).subscribe(
          () => {
            // Data successfully deleted, no specific 'success' property
            this.openDialog(true, 'Exam data deleted successfully!');
            this.getAllCgaRegister(); // Refresh the data after successful deletion
