...
export class ListComponent implements OnInit {
  ...
  columnsToDisplay = ['sno', 'empId', 'empName', 'cadreCode', 'cadreName', 'cadreCategory', 'action'];
  ...
  ngOnInit(): void {
    ...
    this.getAllDirectRecruitment();

    this.activeRoute.paramMap.subscribe(params => {
      this.id = params.get('id');
      this.deleteDirectRecruitment;
  ...
  getAllDirectRecruitment(): void {
    const id = 123; // Replace with the actual employee ID
    this.employeeService.getAllDirectRecruitment(id).subscribe(
      (response) => {
  ...
  deleteDirectRecruitment(id: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        ...
        this.employeeService.deleteDirectRecruitment(id).subscribe(
          () => {
            // Data successfully deleted, no specific 'success' property
            this.openDialog(true, 'Exam data deleted successfully!');
            this.getAllDirectRecruitment(); // Refresh the data after successful deletion
