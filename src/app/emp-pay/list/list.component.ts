...
export class ListComponent implements OnInit {
  ...
  columnsToDisplay = ['sno', 'employeeId', 'employeeName', 'cadreCode', 'cadreName', 'category', 'action'];
  ...
  getAllPay(): void {
    ...
    this.employeeService.getAllPay(id).subscribe(
      (response) => {
  ...
  deletePayFixation(id: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        ...
        this.employeeService.deletePayFixation(id).subscribe(
          () => {
            // Data successfully deleted, no specific 'success' property
            this.openDialog(true, 'Exam data deleted successfully!');
            this.getAllPay(); // Refresh the data after successful deletion
