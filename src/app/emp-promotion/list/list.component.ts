...
export interface AdditionalRows {
  ...
  empId: string;
  empName: string;
  promotedCadreCode: string;
  promotedCadreName : string;
  promotedCadreCategory:string;
  action: string;
...
export class ListComponent implements OnInit {
  ...
  columnsToDisplay = ['sno', 'employeeId', 'employeeName', 'promotedCadreCode', 'promotedCadreName', 'promotedCadreCategory', 'action'];
  ...
  getAllPromotionHistory(): void {
    const id = 123; // Replace with the actual employee ID
    this.employeeService.getAllPromotionHistory(id).subscribe(
      (response) => {
  ...
  deletePromotion(id: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        ...
        this.employeeService.deletePromotion(id).subscribe(
          () => {
            // Data successfully deleted, no specific 'success' property
            this.openDialog(true, 'Exam data deleted successfully!');
            this.getAllPromotionHistory(); // Refresh the data after successful deletion
