...
export class ListComponent implements OnInit {
  ...
  columnsToDisplay = ['sno', 'employeeId', 'employeeName', 'cadreCode', 'cadreName', 'category', 'presentOfficeCode','transferOfficeCode', 'dateOfTransferOrder', 'action'];
  ...
  //   { columnDef: 'cadCate',     header: 'Cade cadrecadreCategory',   cell: (element: any) => 'empId'     },
  ...
  constructor(public dialog: MatDialog, private snackbar: MatSnackBar, private activeRoute: ActivatedRoute, private router: Router,private employeeService:EmployeeService) {
  ...
  ngOnInit(): void {
    ...
    this.getAllTenureHistory();
    // this.deleteTenureRecord;
  ...
  getAllTenureHistory(): void {
    const id = 123; // Replace with the actual employee ID
    this.employeeService.getAllTenureHistory(id).subscribe(
      (response) => {
  ...
  applyTypeFilter() {
    if (this.selectedStatus?.length || this.selectedValue?.length) {
      this.filteredData = this.dataSource.data.filter(item => {
        // Check if the item's cadrecadreCategory is included in the selectedValue array
  ...
  deleteTenureRecord(id: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        ...
        this.employeeService.deleteTenureRecord(id).subscribe(
          () => {
            // Data successfully deleted, no specific 'success' property
            this.openDialog(true, 'Exam data deleted successfully!');
            this.getAllTenureHistory(); // Refresh the data after successful deletion
