...
export class ListComponent implements OnInit {
  ...
  columnsToDisplay = ['sno', 'empId', 'empName', 'nocPurpose', 'applDat', 'status', 'action'];
  ...
  getAllEmpNOC(): void {
    ...
    this.employeeService.getAllEmpNOC(id).subscribe(
      (response) => {
  ...
  deleteEmpNOC(id: number) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === true) {
        ...
        this.employeeService.deleteEmpNOC(id).subscribe(
          () => {
            // Data successfully deleted, no specific 'success' property
            this.openDialog(true, 'Exam data deleted successfully!');
            this.getAllEmpNOC(); // Refresh the data after successful deletion
